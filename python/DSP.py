...
def eyediagrams(data1,data2, n, offset, period, color1):
    span     = 2*n
    segments = int(len(data1)/span)
    xmax     = (n-1)*period
    xmin     = -(n-1)*period
    x        = list(np.arange(-n,n,)*period)
    xoff     = offset
    ...
    for i in range(0,segments-1):
        ax1.plot(x, data1[(i*span+xoff):((i+1)*span+xoff)],color1)
        ax1.hold(True)
    ax1.grid(True)
    ax1.set_title('Eye Q')
    ...
    for i in range(0,segments-1):
        ax2.plot(x, data2[(i*span+xoff):((i+1)*span+xoff)],color1)
        ax2.hold(True)             
    ax2.grid(True)
    ax2.set_title('Eye I')
